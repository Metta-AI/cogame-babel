## Babel entrypoint: reads the Coworld runtime contract and starts either
...
proc randomSeed(): int =
  ...
  if not urandom(buf):
    raise newException(BabelError, "OS entropy source unavailable")
...
when isMainModule:
  ...
  if runtimeConfig.replayMode:
    ...
    if not seedPinned(runtimeConfig.config):
      ## An unpinned seed is randomized so the alphabet, the per-seat
      ## views, the schedule, the aliases, and the scripted-baseline
      ## tie-breaks are not precomputable.
      config.seed = randomSeed()
      echo "babel: seed not pinned; randomized"
